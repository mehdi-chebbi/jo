...
const HomePage = ({ offers }: { offers: Offer[] }) => {
  ...
  const filteredOffers = useMemo(() => {
    return offers.filter(offer => {
      ...
      return matchesLanguage && matchesCategory && matchesCountry && matchesStatus;
    });
  }, [offers, filters.category, filters.country, filters.status, lang]);
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {filters.category && (
                <span className="inline-flex items-center px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm">
                  {t('home.category')}: {getOfferTypeName(filters.category, lang)}
                  <button 
