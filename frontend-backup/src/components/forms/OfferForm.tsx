...
const OfferForm = ({ offer, onSave, onCancel }: { offer?: Offer; onSave: (offer: Offer) => void; onCancel: () => void }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    method: offer?.method || 'appel_d_offres_international',
    title: offer?.title || '',
...
useEffect(() => {
  ...
  // Get default required documents for the offer type (method-based)
  const getDefaultRequiredDocuments = () => {
    const allDocuments = getRequiredDocumentsForMethod(formData.method);
  ...
  const getAllDefaultDocuments = () => {
    return getRequiredDocumentsForMethod(formData.method);
  };
...
console.log('currentStep:', currentStep);
    // Validate that a department is selected (project is optional)
...
console.log('formData at submit:', formData);
    ...
    // Add project_id (optional)
    if (selectedProjectId) {
      formDataToSend.append('project_id', selectedProjectId);
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <select
              ...
              required
            >
              {getOfferTypeOptions(lang).map(option => (
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              required
            >
              {getOfferMethodOptions(lang).map(option => (
                <option key={option.value} value={option.value}>{option.label}</option>
              ))}
            </select>
          </div>
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div>
      <input
        type="date"
        id="deadline-date"
        className="block w-full border-2 border-gray-200 rounded-lg shadow-sm py-3 px-4 text-gray-700 focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 hover:border-gray-300"
        value={formData.deadline ? formData.deadline.split('T')[0] : ''}
        onChange={(e) => {
          const date = e.target.value;
          const time = formData.deadline ? formData.deadline.split('T')[1] : '00:00';
    ...
    <div className="grid grid-cols-2 gap-2">
      ...
      <div>
        <select
          className="block w-full border-2 border-gray-200 rounded-lg shadow-sm py-3 px-4 text-gray-700 focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 hover:border-gray-300"
          value={formData.deadline ? formData.deadline.split('T')[1]?.split(':')[0] || '00' : '00'}
          onChange={(e) => {
            const date = formData.deadline ? formData.deadline.split('T')[0] : new Date().toISOString().split('T')[0];
            const minutes = formData.deadline ? formData.deadline.split('T')[1]?.split(':')[1] || '00' : '00';
            setFormData(prev => ({ ...prev, deadline: `${date}T${e.target.value}:${minutes}` }));
          }}
          required
        >
      ...
      <div>
        <select
          className="block w-full border-2 border-gray-200 rounded-lg shadow-sm py-3 px-4 text-gray-700 focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 hover:border-gray-300"
          value={formData.deadline ? formData.deadline.split('T')[1]?.split(':')[1] || '00' : '00'}
          onChange={(e) => {
            const date = formData.deadline ? formData.deadline.split('T')[0] : new Date().toISOString().split('T')[0];
            const hours = formData.deadline ? formData.deadline.split('T')[1]?.split(':')[0] || '00' : '00';
            setFormData(prev => ({ ...prev, deadline: `${date}T${hours}:${e.target.value}` }));
          }}
          required
        >
