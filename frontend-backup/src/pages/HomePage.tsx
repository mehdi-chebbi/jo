...
import { getOfferTypeName, getOfferMethodName } from '../utils/translations';
...
const HomePage = ({ offers }: { offers: Offer[] }) => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters.search, filters.method, filters.country, filters.status]);
  ...
  const filteredOffers = useMemo(() => {
    return offers.filter(offer => {
      ...
      if (filters.status === 'actif') {
        ...
        // Show both 'resultat' and 'infructueux' when 'resultat' is selected
        matchesStatus = offer.status === 'resultat' || offer.status === 'infructueux';
      }

      return matchesLanguage && matchesMethod && matchesCountry && matchesStatus;
    });
  }, [offers, filters.method, filters.country, filters.status, lang]);
  ...
  // Apply search filter on the display offers
  ...
  // Updated status options - only 3 values
  const statusOptions = [
